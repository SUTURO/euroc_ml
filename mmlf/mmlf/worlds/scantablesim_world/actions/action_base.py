...
class SimSimAction(object):
    ...
    def move_to(self, x, y, env):
        ...
        while not (x >= 0 and y >= 0 and x < env.configDict["rows"] and y < env.configDict["columns"]):
            x += 1 if x < curr_x else -1
            y += 1 if y < curr_y else -1
        env.pos_x = x
        env.pos_y = y
    ...
    def scan_table2(self,env):
        ...
        # if r > 0:
        #     print r
        return 100 if not env.currentState["isScanned"] else -10
