...
class ScanTableSimulation(object):
    ...
    def see(self):
        #000
        result = 0
        if self.camera_index > 0 and self.table_cells[self.camera_index-1]:
            result += 4
        if self.table_cells[self.camera_index]:
            result += 2
        if self.camera_index < len(self.table_cells)-1 and self.table_cells[self.camera_index+1]:
            result += 1
        return result
